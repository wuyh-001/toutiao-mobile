<template>
  <div class="article-comments">
    ...
    <van-list
      ...
      @load="onLoad"
    >
        <recommend-item v-for="(item, index) in list" :key="index" :comment="item" @reply-click="$emit('replayEvent',$event)"></recommend-item>
...
components:{
  ...
  methods: {
    async onLoad() {
      ...
      this.loading = false;
      this.$emit('update-comment-total',data.data.total_count)
      // 4. 判断是否还有数据
      if (results.length) {
